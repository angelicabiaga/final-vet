...
const getBirthdayDateFromDraft = (draft) => {
  const monthIndex = Math.max(MONTHS.indexOf(draft?.birthMonth), 0);
  const day = Math.max(Number(draft?.birthDay) || 1, 1);
  const year = Number(draft?.birthYear) || Number(YEARS[0]);
...
const StaffPetsProfileEdit = ({ navigation, route }) => {
  ...
  const [draftPet, setDraftPet] = useState(() => createStaffEditDraft(null));
  ...
  const syncDraftFromStore = useCallback(() => {
    const staffPet = getStaffPets().find((pet) => pet.id === petId) || null;
    ...
    const nextDraft = createStaffEditDraft(staffPet);
    setDraftPet(nextDraft);
    setIsCustomBreedMode(() => (
      Boolean(nextDraft.breed) &&
      !getBreedOptionsForSpecies(nextDraft.species).some((item) => item.value === nextDraft.breed)
    ));
  }, [loggedInUser, navigation, petId]);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      syncDraftFromStore();
    }, [loggedInUser, navigation, petId, syncDraftFromStore]),
  );
  ...
  const birthdayDate = useMemo(() => getBirthdayDateFromDraft(draftPet), [draftPet.birthMonth, draftPet.birthDay, draftPet.birthYear]);
  const ageLabel = useMemo(() => {
    const computed = getFormattedAgeFromBirthday({
      birthMonth: draftPet.birthMonth,
      birthDay: draftPet.birthDay,
      birthYear: draftPet.birthYear,
    });
    return computed || draftPet.age || '-';
  }, [draftPet.age, draftPet.birthDay, draftPet.birthMonth, draftPet.birthYear]);
  ...
  const openBirthdayCalendar = () => {
    setCalendarMonthDate(getBirthdayDateFromDraft(draftPet));
  ...
  const handleSavePet = () => {
    if (hasIncompletePetDraft(draftPet)) {
      setShowDoneConfirm(false);
      setShowRequiredFieldsModal(true);
      return;
    }

    const result = saveStaffPetFromDraft(draftPet);
    ...
    const finalPetId = result?.savedPet?.id || draftPet.id;
    ...
    <LinearGradient colors={['#022c42', '#0c212b', '#15394e']} style={styles.background}>
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.headerTopRow}>
            ...
            >
              ...
              <View style={styles.brandBlock}>
                ...
                <Text style={styles.headerSubtitle}>Edit Pet Profile</Text>
              </View>
            </TouchableOpacity>
          ...
          <Animated.View style={[styles.headerBottomRowWrap, lowerHeaderAnimatedStyle]}>
            ...
            <View style={styles.ownerSummary}>
              <Text style={styles.headerCaption}>Patient records</Text>
        ...
        >
          <View style={styles.sectionHeaderWrap}>
            <Text style={styles.sectionTitle}>Edit Pet Profile</Text>
            <Text style={styles.sectionSubtitle}>Update pet photo, clinical details, and staff records</Text>
          </View>
          ...
          <View style={styles.detailCard}>
              ...
              <View style={styles.editAvatarWrap}>
                <View style={[styles.largePetAvatar, { backgroundColor: draftPet.profileColor }]}>
                  {activePhoto.source ? (
                    ...
                    <Text style={styles.largePetAvatarText}>{(draftPet.name || 'P').charAt(0)}</Text>
                  )}
                </View>
            ...
            <View style={styles.formCard}>
              ...
              <Text style={styles.sectionSubtitle}>One line per item for lists below</Text>

              <Text style={styles.formLabel}>Medical history</Text>
              <TextInput
                value={draftPet.medicalHistoryText}
                onChangeText={(value) => updateDraftPetField('medicalHistoryText', value)}
                style={styles.textAreaField}
                placeholder="Enter one medical history item per line"
                placeholderTextColor="#87a0b1"
                multiline
                textAlignVertical="top"
              />

              <Text style={styles.formLabel}>Vaccination records</Text>
              <TextInput
                value={draftPet.vaccinationRecordsText}
                onChangeText={(value) => updateDraftPetField('vaccinationRecordsText', value)}
                style={styles.textAreaField}
                placeholder="Enter one vaccine record per line"
                placeholderTextColor="#87a0b1"
                multiline
                textAlignVertical="top"
              />

              <Text style={styles.formLabel}>Visit history</Text>
              <TextInput
                value={draftPet.visitsText}
                onChangeText={(value) => updateDraftPetField('visitsText', value)}
                style={styles.textAreaField}
                placeholder="Enter one visit entry per line"
                placeholderTextColor="#87a0b1"
                multiline
                textAlignVertical="top"
              />

              <Text style={styles.formLabel}>Veterinarian notes</Text>
              <TextInput
                value={draftPet.veterinarianNotesText}
                onChangeText={(value) => updateDraftPetField('veterinarianNotesText', value)}
                style={styles.textAreaField}
                placeholder="Enter one staff note per line"
                placeholderTextColor="#87a0b1"
                multiline
                textAlignVertical="top"
              />
...
export default StaffPetsProfileEdit;
