...
const getNextAccountUid = (accounts) => {
  const nextNumber = accounts.reduce((highest, account) => {
    const parsedNumber = Number(String(account.uid || '').replace(/[^\d]/g, ''));
...
const StaffCreateAccount = ({ navigation, route }) => {
  const loggedInUser = route?.params?.user;
  const existingAccounts = route?.params?.existingAccounts || [];
  ...
  const [accountForm, setAccountForm] = useState(() => buildAccountForm());
  ...
  const handleCreateAccount = () => {
    if (hasEmptyRequiredField(accountForm)) {
      setFormError('Please fill in all required fields before creating this account.');
      return;
    }
    ...
    const nextAccount = {
      ...
      uid: getNextAccountUid(existingAccounts),
      ...
      <SafeAreaView style={localStyles.container}>
          ...
          >
            <View style={dashboardStyles.headerBottomRow}>
              ...
              <View style={dashboardStyles.ownerSummary}>
                <Text style={dashboardStyles.headerCaption}>New pet owner account</Text>
        ...
        >
          <View style={localStyles.sectionHeaderWrap}>
            <Text style={localStyles.sectionTitle}>Create Pet Owner Account</Text>
            <Text style={localStyles.sectionSubtitle}>Add the username first, then complete the profile, contact details, and status activity.</Text>
          </View>
          ...
          <View style={localStyles.formCard}>
            ...
            <View style={localStyles.fieldGroup}>
              ...
              <Text style={localStyles.fieldHelperText}>New pet owner accounts usually start as active.</Text>
            ...
            <View style={localStyles.actionRow}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.9}
              >
                <Text style={localStyles.createButtonText}>Create Account</Text>
              </TouchableOpacity>
            </View>
          </View>
        </ScrollView>
