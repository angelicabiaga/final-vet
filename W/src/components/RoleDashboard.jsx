...
export default function RoleDashboard({ profile }) {
  ...
  const summary = useMemo(() => {
    ...
    const activeQueues = data.queues.filter((q) => ACTIVE_QUEUE.includes(q.queue_status));
    ...
    if (profile?.role === "admin") {
      ...
      cards = [
        ...
        [Clock3, "Waiting Queue", activeQueues.filter((q) => q.queue_status === "Waiting").length, `${activeQueues.length} active entries`],
      ...
      {loading ? <div className="dash-loading"><RefreshCw className="spin"/> Loading dashboard...</div> : (
        <>
          ...
          <div className="dash-grid">
            ...
            <section className="dash-panel"><header><div><h3>Live Queue</h3><p>Current active queue entries</p></div><Clock3 size={20}/></header><div className="list">{summary.queues.length ? summary.queues.map((q) => <div key={q.id}><b>{q.queue_number || "Queue"}</b><span>{q.queue_status}</span></div>) : <em>No active queue entries.</em>}</div></section>
          </div>
