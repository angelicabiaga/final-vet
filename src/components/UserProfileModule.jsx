...
export default function UserProfileModule({ profile, title = "My Profile" }) {
  ...
  async function saveDetails(event) {
    ...
    try {
      const result = await requestProfileUpdate(profile.id, { ...form, full_name: joinFullName(form) });
  ...
  async function chooseAvatar(event) {
    ...
    try {
      ...
      const updated = await updateProfile(profile.id, { ...form, full_name: joinFullName(form), avatar_url });
  ...
  return <AppShell profile={profile} title={title}><div className="profilePage">
    ...
    {loading ? <div className="card">Loading profile...</div> : <div className="columns">
      <form className="card form" onSubmit={saveDetails}><h3><UserCircle size={20}/> Personal Information</h3>
        ...
        <label><span>Middle name<span className="required">*</span></span><input value={form.middleName} onChange={(e)=>field("middleName",e.target.value)} required/></label>
