...
const PET_FIELDS="id,owner_id,pet_name,species,breed,sex,date_of_birth,weight,color,microchip_number,allergies,existing_conditions,notes,photo_url,is_archived,created_at,updated_at,owner:profiles!pets_owner_id_fkey(id,full_name,email,phone,address)";
...
export async function getPetOwnersDirectory(){
  const{data,error}=await supabase.from("profiles").select("id,full_name,username,email,phone,address").eq("role","pet_owner").eq("account_status","active").order("full_name");
