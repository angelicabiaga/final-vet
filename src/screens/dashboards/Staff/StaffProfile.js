...
const resolveStaffRole = (user) => {
  const roleValue = user?.department || user?.position || user?.role || 'staff';

  if (String(roleValue).toLowerCase() === 'staff') {
    return 'Clinic Staff';
  }
...
const resolveStaffId = (user) => {
  ...
  if (rawId) {
    return `STF-${String(rawId).slice(-6).toUpperCase()}`;
  }
...
const buildProfileFromUser = (user) => {
  ...
  const resolvedName = directName || fallbackName || '';
  ...
  return {
    ...
    department: resolveStaffRole(user),
    staffId: resolveStaffId(user),
...
const StaffProfile = ({ navigation, route }) => {
  ...
  const draftProfileImageUri = draftProfile?.profileImageUri || '';
  ...
  const pickPhotoFromAlbum = async () => {
    try {
      ...
      console.warn('Failed to open album picker for staff profile photo:', error);
    }
  };
  ...
  const pickPhotoFromFiles = async () => {
    try {
      ...
      console.warn('Failed to open file picker for staff profile photo:', error);
    }
  };
  ...
  const takePhotoWithCamera = async () => {
    try {
      ...
      console.warn('Failed to open camera for staff profile photo:', error);
    }
  };
          ...
          {showProfileToast ? (
            <View style={styles.notificationToast}>
              ...
              <Text style={styles.notificationToastText}>
                Your staff profile changes were saved successfully.
              </Text>
            </View>
          ) : null}
        ...
        >
          <View style={styles.sectionHeaderWrap}>
            ...
            <Text style={styles.sectionSubtitle}>
              {isEditing
                ? 'Update your staff details below'
                : 'Main staff information and quick account summary'}
            </Text>
          </View>
            ...
            <View style={styles.profileTopRow}>
              ...
              <View style={styles.profileTopContent}>
                ...
                <Text style={styles.profileMeta}>
                  {profileData.department || 'Clinic Staff'}{profileData.staffId ? ' - ' + profileData.staffId : ''}
                </Text>
            ...
            {isEditing ? (
              ...
              <View style={styles.infoGrid}>
                ...
                <View style={styles.infoItem}>
                  ...
                  <Text style={styles.infoValue}>{profileData.department || 'Clinic Staff'}</Text>
                </View>
                <View style={styles.infoItem}>
                  <Text style={styles.infoLabel}>Staff ID</Text>
                  <Text style={styles.infoValue}>{profileData.staffId || 'Not set'}</Text>
                </View>
              </View>
            )}
          </View>
          ...
          <View style={styles.sectionHeaderWrap}>
            ...
            <Text style={styles.sectionSubtitle}>
              Update your staff account or safely sign out
            </Text>
          </View>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.photoModalCard}>
              ...
              <Text style={styles.modalMessage}>
                Choose how you want to add your staff profile picture.
              </Text>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              ...
              <Text style={styles.modalMessage}>
                Are you sure you want to save these staff profile changes?
              </Text>
