...
const PAYMENT_METHODS = ["Cash", "GCash", "Maya", "Credit Card", "Debit Card", "Split Payment"];
const PAYMENT_STATUSES = ["", "Paid", "Pending", "Voided", "Refunded", "Cancelled"];
  ...
  async function confirmReversal() {
    ...
    try {
      await reverseTransaction({
        ...
        itemIds: actionDialog.itemIds || null,
      }, profile);
      ...
      setError(reversalError.message || "Unable to reverse this transaction.");
    } finally {
      setReversing(false);
    }
  }
    ...
    <section className="history-module" id="payment-history">
      ...
      <div className="history-table-wrap">
        <table className="history-table">
          ...
          <tbody>
            ...
            {!loading && visibleTransactions.map((transaction) => <tr key={transaction.id}>
              ...
              <td><div className="row-actions"><button type="button" className="table-action" onClick={() => openDetails(transaction)}><Eye size={16} /> Details</button><button type="button" className="table-action" onClick={() => printReceipt(transaction)}><Printer size={16} /> Reprint</button></div></td>
            </tr>)}
          </tbody>
        </table>
      </div>
      ...
      {details && <div className="receipt-overlay" role="dialog" aria-modal="true" aria-label="Transaction details">
        <div className="transaction-detail-card">
          ...
          <div className="detail-items"><h3>Items & services</h3>{receiptRows(details).map((item) => <div className="detail-item" key={item.id}><span><b>{item.item_name}</b><small>{item.quantity} × {money(item.unit_price)}{item.refunded_quantity ? ` · ${item.refunded_quantity} refunded` : ""}</small></span><strong>{money(item.line_total)}</strong>{canReverse && details.payment_status === "Paid" && item.id !== "checkup" && Number(item.refunded_quantity || 0) < Number(item.quantity || 0) && <button type="button" className="inline-refund" onClick={() => { setActionDialog({ transaction: details, action: "Refund", itemIds: [item.id], label: `Refund ${item.item_name}` }); setReversalReason(""); }}><Undo2 size={14} /> Refund item</button>}</div>)}</div>
          <div className="detail-totals"><div><span>Subtotal</span><span>{money(details.subtotal)}</span></div><div><span>Discount</span><span>-{money(details.discount_amount)}</span></div><div><span>Total amount</span><strong>{money(details.total_amount)}</strong></div><div><span>Amount paid</span><span>{money(details.amount_paid)}</span></div><div><span>Change</span><span>{money(details.change_amount)}</span></div>{Number(details.refunded_amount || 0) > 0 && <div><span>Refunded</span><span>-{money(details.refunded_amount)}</span></div>}</div>
          ...
          <div className="detail-actions"><button type="button" className="receipt-print" onClick={() => printReceipt(details)}><Printer size={17} /> Reprint Receipt</button>{canReverse && details.payment_status === "Paid" && <><button type="button" className="refund-button" onClick={() => { setActionDialog({ transaction: details, action: "Refund", label: "Refund full transaction" }); setReversalReason(""); }}><Undo2 size={17} /> Refund</button><button type="button" className="void-button" onClick={() => { setActionDialog({ transaction: details, action: "Void", label: "Void transaction" }); setReversalReason(""); }}><RotateCcw size={17} /> Void</button></>}{canReverse && details.payment_status === "Pending" && <button type="button" className="void-button" onClick={() => { setActionDialog({ transaction: details, action: "Void", label: "Void pending transaction" }); setReversalReason(""); }}><RotateCcw size={17} /> Void</button>}</div>
        </div>
      </div>}

      {actionDialog && <div className="receipt-overlay" role="dialog" aria-modal="true" aria-label={`${actionDialog.action} transaction`}><div className="reversal-card"><h2>{actionDialog.label}</h2><p>{actionDialog.transaction.or_number} · {money(actionDialog.transaction.total_amount)}</p><label>Reason <textarea value={reversalReason} onChange={(event) => setReversalReason(event.target.value)} placeholder="Required reason for this audit action" autoFocus /></label><div><button type="button" className="modal-cancel" onClick={() => setActionDialog(null)}>Cancel</button><button type="button" className="modal-confirm" disabled={!reversalReason.trim() || reversing} onClick={confirmReversal}>{reversing ? "Saving…" : `Confirm ${actionDialog.action}`}</button></div></div></div>}
    </section>
  );
}
...
export default function TransactionManagement({ profile }) {
  ...
  const discountAmount = applyDiscount ? subtotal * 0.2 : 0;
  ...
  function addToCart(inventoryItem) {
    setCart((current) => {
      const currentLine = current.find((line) => line.inventory_item_id === inventoryItem.id);
      if (currentLine) return current.map((line) => line.inventory_item_id === inventoryItem.id ? { ...line, quantity: Number(line.quantity) + 1 } : line);
      return [...current, { inventory_item_id: inventoryItem.id, item_name: inventoryItem.item_name, item_type: guessItemType(inventoryItem), quantity: 1, unit_price: Number(inventoryItem.unit_price || 0), available: Number(inventoryItem.quantity || 0), deduct_inventory: true }];
    });
    setItemSearch(""); setItemResults([]); setItemFocused(false);
  }
  function updateCartLine(inventoryItemId, patch) { setCart((current) => current.map((line) => line.inventory_item_id === inventoryItemId ? { ...line, ...patch } : line)); }
  function removeCartLine(inventoryItemId) { setCart((current) => current.filter((line) => line.inventory_item_id !== inventoryItemId)); }
  function resetForm() { setSelectedPet(null); setIncludeCheckupFee(true); setCheckupFee("500"); setApplyDiscount(false); setPaymentMethod("Cash"); setAmountPaid(""); setAmountTouched(false); setSplitPayment({ cash: "", digital: "", digitalMethod: "GCash" }); setNotes(""); setCart([]); setError(""); }
  ...
  return <AppShell profile={profile} title="Transaction Management"><div className="transaction-module">
    {error && <div className="error">{error}</div>}
    <div className="pos-grid"><section className="card pos-main"><h2><Stethoscope size={22} /> New Transaction</h2>
      ...
      <label className="field-label"><PackageSearch size={16} /> Add tests / medicines / products from inventory</label>
      <div className="search-box"><Search size={18} /><input placeholder="Search inventory by item name or SKU" value={itemSearch} onChange={(event) => setItemSearch(event.target.value)} onFocus={() => setItemFocused(true)} onBlur={() => window.setTimeout(() => setItemFocused(false), 150)} /></div>
      {(itemSearch.trim() || itemFocused) && <div className="results-list">{itemLoading && <div className="results-empty">Searching…</div>}{!itemLoading && itemResults.length === 0 && <div className="results-empty">No inventory items found.</div>}{itemResults.map((item) => <button type="button" key={item.id} className="result-row" disabled={Number(item.quantity) <= 0} onClick={() => addToCart(item)}><span><strong>{item.item_name}</strong> · {item.category}</span><span className="muted">{money(item.unit_price)} · {item.quantity} {item.unit} in stock</span></button>)}</div>}

      <label className="field-label"><ShoppingCart size={16} /> Cart</label>
      {cart.length === 0 ? <div className="results-empty cart-empty"><ShoppingCart size={18} /> No tests, medicines, or products added yet.</div> : <div className="cart-scroll"><table className="cart-table"><thead><tr><th>Item</th><th>Type</th><th>Qty</th><th>Unit Price</th><th>Line Total</th><th /></tr></thead><tbody>{cart.map((line) => <tr key={line.inventory_item_id}><td>{line.item_name}</td><td>{line.item_type}</td><td><div className="qty-stepper"><button type="button" onClick={() => updateCartLine(line.inventory_item_id, { quantity: Math.max(1, Number(line.quantity) - 1) })}><Minus size={14} /></button><input type="number" min="1" max={line.available} value={line.quantity} onChange={(event) => updateCartLine(line.inventory_item_id, { quantity: event.target.value })} /><button type="button" onClick={() => updateCartLine(line.inventory_item_id, { quantity: Math.min(line.available, Number(line.quantity) + 1) })}><Plus size={14} /></button></div></td><td>{money(line.unit_price)}</td><td>{money(Number(line.quantity) * Number(line.unit_price))}</td><td><button type="button" className="icon-btn" onClick={() => removeCartLine(line.inventory_item_id)} aria-label={`Remove ${line.item_name}`}><Trash2 size={17} /></button></td></tr>)}</tbody></table></div>}
      ...
      <label className="field-label toggle-label discount-toggle"><input type="checkbox" className="inline-checkbox" checked={applyDiscount} onChange={(event) => setApplyDiscount(event.target.checked)} /> Apply PWD / Senior Citizen Discount (20%)</label>
      ...
      <div className="totals"><div><span>Service subtotal</span><span>{money(effectiveCheckupFee)}</span></div><div><span>Items subtotal</span><span>{money(itemsSubtotal)}</span></div><div><span>Subtotal</span><span>{money(subtotal)}</span></div>{applyDiscount && <div><span>PWD / Senior discount</span><span>-{money(discountAmount)}</span></div>}<div className="grand-total"><span>Total amount due</span><span>{money(totalAmount)}</span></div></div>
