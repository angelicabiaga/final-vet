    ...
    <section className="history-module" id="payment-history">
      ...
      {details && <div className="receipt-overlay" role="dialog" aria-modal="true" aria-label="Transaction details">
        <div className="transaction-detail-card">
          ...
          <div className="detail-totals"><div><span>Subtotal</span><span>{money(details.subtotal)}</span></div><div><span>Discount</span><span>-{money(details.discount_amount)}</span></div><div><span>Total amount</span><strong>{money(details.total_amount)}</strong></div><div><span>Amount paid</span><span>{money(details.amount_paid)}</span></div><div><span>Change</span><span>{money(details.change_amount)}</span></div></div>
