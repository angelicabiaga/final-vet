...
export default function UserManagement({ profile }) {
  ...
  async function load(){setLoading(true);setError("");try{setUsers(await fetchUsers({search,role,status}));}catch(e){setError(e.message);}finally{setLoading(false);}}
  useEffect(()=>{load();},[]);
