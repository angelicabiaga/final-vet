...
export default function UserManagement({ profile }) {
  ...
  return <AppShell profile={profile} title="User Management"><div className="um">
    ...
    {showForm&&<div className="overlay"><form className="modal" onSubmit={submit}><button type="button" className="close" onClick={()=>setShowForm(false)}><X/></button><ShieldCheck size={34}/><h3>Create Staff or Veterinarian</h3><div className="grid">
      <label><span>First name<span className="required-mark"> *</span></span><input required value={form.firstName} onChange={e=>setForm({...form,firstName:e.target.value})}/></label>
      <label><span>Last name<span className="required-mark"> *</span></span><input required value={form.lastName} onChange={e=>setForm({...form,lastName:e.target.value})}/></label>
      <label><span>Middle name<span className="optional-mark"> (Optional)</span></span><input value={form.middleName} onChange={e=>setForm({...form,middleName:e.target.value})}/></label>
      <label><span>Username<span className="required-mark"> *</span></span><input required value={form.username} onChange={e=>setForm({...form,username:e.target.value})}/></label>
